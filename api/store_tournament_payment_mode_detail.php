...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $result = $organizer->storeTournamentPaymentModeDetail($data);

    if($result['error'] != NULL){

        $status = FALSE;    
        $message = 'Fail! Tournament payment mode details are not saved.';
        $data = $result['error'];
        $code = 400;
        ...
        if(empty($result['data'])){
            ...
            unset($result['data']);
            $status = TRUE;
            $message = 'fail! Tournament payment mode details are already saved for this tournament id.';
            $data = [];
            $code = 200;
            echo json_encode(['ResponseCode'=>$code,'ResponseType'=>$status,'ResponseMessage'=>$message,'ResponseData'=>$data],$code);
        }else{
            unset($result['error']);
            $status = TRUE;
            $message = 'Success! Tournament payment mode details are saved.';
            $data = $result['data'];
            $code = 200;
            echo json_encode(['ResponseCode'=>$code,'ResponseType'=>$status,'ResponseMessage'=>$message,'ResponseData'=>$data],$code);
        } 
