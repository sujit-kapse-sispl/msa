...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $result = $organizer->getTournamentWhatsAppDetail($data);
    
    if($result['error'] != NULL){
        $status = FALSE;
        $message = 'Fail! Tournament whats app link are not found.';
        $data = $result['error'];
        $code = 400;
        ...
        unset($result['error']);
        $status = TRUE;
        $message = 'Success! Tournament whats app link are found.';
        $data = $result['data'];
        $code = 200;
        echo json_encode(['ResponseCode'=>$code,'ResponseType'=>$status,'ResponseMessage'=>$message,'ResponseData'=>$data],$code);
    } 
