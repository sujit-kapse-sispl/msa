...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $result = $organizer->getTournamentHospitalHotelDetail($data);

    if($result['error'] != NULL){
        $status = FALSE;    
        $message = 'Fail! Tournament hospital hotel details are not found.';
        $data = $result['error'];
        $code = 400;
        ...
        if(empty($result['data'])){
            ...
            unset($result['error']);
            $status = TRUE;
            $message = 'Success! Tournament hospital hotel details are found.';
            $data = $result['data'];
            $code = 200;
            echo json_encode(['ResponseCode'=>$code,'ResponseType'=>$status,'ResponseMessage'=>$message,'ResponseData'=>$data],$code);
        } 
