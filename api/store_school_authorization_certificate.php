...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $result = $organizer->storeSchoolAuthorizationCertificateDetail($data);

    if($result['error'] != NULL){

        $status = FALSE;    
        $message = 'Fail! School authorization certificate details are not saved.';
        $data = $result['error'];
        $code = 400;
        ...
        if(empty($result['data'])){
            ...
            unset($result['data']);
            $status = TRUE;
            $message = 'fail! School authorization certificate details are already saved for this tournament id.';
            $data = [];
            $code = 200;
            echo json_encode(['ResponseCode'=>$code,'ResponseType'=>$status,'ResponseMessage'=>$message,'ResponseData'=>$data],$code);
        }else{
            unset($result['error']);
            $status = TRUE;
            $message = 'Success! School authorization certificate details are saved.';
            $data = $result['data'];
            $code = 200;
            echo json_encode(['ResponseCode'=>$code,'ResponseType'=>$status,'ResponseMessage'=>$message,'ResponseData'=>$data],$code);
        } 
